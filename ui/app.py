...
try:
    ...
    logfire.configure(
        ...
        console=None if token else False,
    )
    ...
    LOGFIRE_STATUS = "Connected & Tracing" if token else "Local only (tracing disabled)"
        ...
        with st.chat_message("assistant", avatar=AI_AVATAR):
            with st.status("🔍 Agent is thinking...", expanded=True) as status:
                try:
                    ...
                    status.update(
                        label=("⚙️ Configuration required" if data.get("status") == "configuration_required" else "✅ Answer Synthesized"),
                        state=("error" if data.get("status") == "configuration_required" else "complete"),
                        expanded=False,
                    )
